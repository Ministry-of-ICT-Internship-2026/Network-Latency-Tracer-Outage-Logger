"""
analytics.py — Analytics engine for the Network Latency Tracer & Outage Logger.

Reads raw ping/outage records out of the SQLite database produced by
database.py and turns them into report-ready metrics: availability,
latency distributions, jitter, outage frequency/duration (MTTR, MTBF),
error-cause breakdown, and time-bucketed series for charting.

This module has no UI/rendering logic — it returns plain dicts/lists so
it can back a static HTML report (report_generator.py), a live
dashboard, or be dropped straight into a notebook / weekly-report
workflow.
"""
...
class MonitoringAnalytics:
    ...
    def _reconstruct_outages(self, rows: List[sqlite3.Row]) -> tuple:
        """
        ...
        # Fill host field (rows don't include host column in this query;
        # caller sets it via closure context — patched below).
